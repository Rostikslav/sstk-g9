    ...
    object PowerManager {
        ...
        suspend fun setTimer (device: EcoSwitch, seconds: Int, action: Int): Boolean {
            return withContext(Dispatchers.IO){
                try{
                    val url = "http://${device.ipAddress}/timer?token=${device.token}&seconds=$seconds&action=$action"
